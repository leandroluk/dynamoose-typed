...
- **DynamoDB Local (the official AWS jar) does not support Streams** — `subscribe()` needs a backend that implements the Streams API: real AWS DynamoDB, or an emulator like [LocalStack](https://www.localstack.cloud/). Against an emulator, pass a fully-configured client via `DataSourceOptions.client` (with `region`, `endpoint`, and dummy `credentials`) instead of `local: true` — `local: true` builds a client with no region/credentials, which the AWS SDK's default provider chain may fail to resolve for the Streams calls. See `.examples/streams` for a working example against LocalStack.
